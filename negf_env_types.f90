...
!> u_L                   : u_L(i) corresponds to -e*mu_L(i), mu_L(i) - 
!>                          chemical potential of the i-th lead
!> EF_L                  : EF_L(i) Fermi level in the i-th lead
!> EF_S                  : Fermi level in the scattering region
!> Temperature           : of electron gas
!> gamma_int             : length of the L-contour
!> delta_int             : height of the L-contour
!> Elow_int              : max lowest energy level of leads/sys
!>                         in respect to the correspondent EF
!> Eup_int               : upper integration limit
!> alpha_R_int           : coef., increases the radius of C-contour (>1)
! ************************************************************************
  TYPE negf_env_data
...
CONTAINS
  ...
  SUBROUTINE negf_env_create(negf_env, nterminals, eps_E, eps_Sancho,&
    ...
    negf_env%obj%Temperature = 2.6E-2_dp  !it is 300 K in eV. Change!
    negf_env%obj%gamma_int   = 1._dp !it is in eV. Change!
    negf_env%obj%delta_int   = 1._dp !it is in eV. Change!
    negf_env%obj%Elow_int    = -10._dp !it is in eV. Change!
    negf_env%obj%Eup_int     = 10._dp !it is in eV. Change! 
...
!REAL(KIND=dp) :: RefF, ImfF
!IF (T == 0._dp) THEN
!!0 K
!  IF (REAL(EE)>EF) THEN
!    fF = CMPLX(0._dp,0._dp)
!  END IF
!  IF (REAL(EE)<EF) THEN
!    fF = CMPLX(1._dp,0._dp)
!  ELSE
!    fF = CMPLX(0.5_dp,0._dp)
!  END IF
!<> 0 K
!ELSE
...
!> do not use it for G(z)f(z) with imag(z) considerably <> 0!
TYPE(negf_env_obj), INTENT(IN) :: negf_env
TYPE(mat_z_obj), INTENT(INOUT) :: G_f
COMPLEX(KIND=dp), INTENT(IN)   :: EE

COMPLEX(KIND=dp)               :: fF, EE_temp
...
! CALL mat_scale(P_G_f, -1.0_dp/pi*fF)
CALL mat_scale(P_G_f, CMPLX(-1.0_dp/pi,0.0_dp,KIND=dp))
...
CALL mat_axpy((1._dp,0._dp), 'N', mat_tmp1, mat_tmp2)!>GammaL*fL + GammaR*fR

CALL calc_G(negf_env, EE, G) !>G
...
CALL mat_mult('N', 'C', (1._dp,0._dp), mat_tmp, G, &
                        (0._dp,0._dp), G_less)!>G*[Gamma_L*fL + Gamma_R*fR)*G'
CALL mat_scale(G_less, (0._dp,1._dp))!>i* ...
